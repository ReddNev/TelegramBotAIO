...
async def get_wallets(callback_query: types.CallbackQuery):
    """
    Возвращает кошельки
    :param :
    :return:
    ...
    await callback_query.message.edit_text('Ваши кошельки', reply_markup=group_btn_s)
...
async def get_wallet_commands(callback_query: types.CallbackQuery):
        ...
        for method_key, method_value in WALLETS_COMMAND.items():
           ...
           # btn_main = InlineKeyboardButton(text='Главное меню', callback_data='get_wallets')
